...
export function SuccessCheckmark({ size = 120 }: SuccessCheckmarkProps) {
  ...
  useEffect(() => {
    // A reduced-motion user still gets the completed mark and the haptic —
    // just instantly, with no draw-in/bounce, not a lesser version.
    if (reducedMotion) {
  ...
  }, [checkProgress, pop, ringProgress, reducedMotion]);
  ...
  return (
    <ReanimatedAnimated.View style={[{ width: size, height: size }, groupStyle]}>
      <Svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
        ...
        <Circle cx={c} cy={c} r={r} stroke={colors.surfaceBorder} strokeWidth={strokeWidth} fill="none" />

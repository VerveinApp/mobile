...
export function WheelPicker({ items, selectedIndex, onChange, width = 90 }: WheelPickerProps) {
  ...
  // Dark mode keeps its exact original band border (0.14 alpha) rather than
  // reusing the shared surfaceBorder token (0.1 alpha) — a tiny difference
  // that would otherwise be a pixel-level dark-mode regression for no reason.
  const bandBorderColor = resolvedScheme === 'dark' ? 'rgba(255,255,255,0.14)' : colors.surfaceBorder;
  const styles = useMemo(() => createStyles(colors, bandBorderColor), [colors, bandBorderColor]);
  const [scrollY] = useState(() => new Animated.Value(selectedIndex * WHEEL_ITEM_HEIGHT));
...
function createStyles(colors: ReturnType<typeof useAppTheme>['colors'], bandBorderColor: string) {

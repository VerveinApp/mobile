...
export default function OnboardingCommitmentScreen() {
  ...
  const { name, goal, experience, environment, sex, heightCm, weightKg, duration, days } = useLocalSearchParams<{
    name?: string;
    goal?: string;
    experience?: string;
    environment?: string;
  ...
  const handleBuildPlan = () => {
    ...
    // through First Look and account creation, so the draft stays alive
    // (now pointing at First Look) rather than being cleared here.
    const params = {
      name: name ?? '',
      goal: goal ?? '',

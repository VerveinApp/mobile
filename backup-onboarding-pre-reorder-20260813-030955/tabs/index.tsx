...
import { loadOnboardingDraft, ONBOARDING_STEP_ROUTES } from '@/lib/onboarding-draft';
...
export default function CreateAccountScreen() {
  ...
  // If the app was killed mid-onboarding (route params carry all other
  // in-progress state and don't survive a process kill), pick back up where
  // the user left off instead of losing their answers to a full restart.
  useEffect(() => {
    let cancelled = false;
  ...
  const handleContinue = () => {
    ...
    router.push({ pathname: '/auth/verify', params: { email: trimmed } } as never);
  };

  // Apple/Google are separate auth paths — they intentionally skip email verification
  // and go straight to onboarding once wired up. Not implemented yet (no backend/SDK
  // integration), so these are placeholders that preserve the correct routing shape.
  const handleAppleAuth = () => {
    hapticImpactLight();
    // TODO: integrate Sign in with Apple, then route to onboarding on success.
    router.push('/onboarding' as never);
  };

  const handleGoogleAuth = () => {
    hapticImpactLight();
    // TODO: integrate Google auth, then route to onboarding on success.
    router.push('/onboarding' as never);
  };
